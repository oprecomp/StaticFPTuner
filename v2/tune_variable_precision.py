...
import math
import utils
import extract_dep_graphs_from_json as dep_graphs
...
smart_tuner_dir = '../'
benchmarks_home = smart_tuner_dir + 'benchmarks'
data_set_dir = smart_tuner_dir + 'precision_and_errors/'
initial_df_dir = opt_dir + 'initial_dfs/'
...
initial_dataset_size = 2000
initial_train_set_size = 1000

# new configurations are given different (greater) weight
new_config_weight = 2

retrain_ML_fromScratch = False
regr_weights_based_on_target = True
use_var_relations = True
...
def solve_opt_model(benchmark, mdl, trgt_error_ratio_log_exp, regr, mae, rmse, 
        ...
        train_set_size, large_err_thresh, n_iter=-1, wrong_configs=[], 
        debug=True, var_rels={}):
    if mdl == None:
        if debug:
        ...
        mdl = create_MP_model(benchmark, bkd, 
                ...
                classr, acc, robust_param, large_err_thresh, debug, var_rels)
...
def create_MP_model(benchmark, bkd, trgt_error_ratio_log_exp, regr, mae, rmse,
    ...
    # use variable relationships extracted from dependency graph
    if var_rels:
        # binary relations Vs, Vg: less or equal
        # e.g. V1 = V2+V3 ~= V1 = T4(V2) + T4(V3) --> T4 <= V1 (if T4 is not
        # used in other other expressions, it would make no sense to allocate to 
        # it more bits than the expression result)
        for vs, vg in var_rels['bin']:
            vs_type = vs[0]
            vs_idx = int(vs[1:])
            vg_type = vg[0]
            vg_idx = int(vg[1:])
            x_vs = mdl.get_var_by_name('x_{}'.format(vs_idx))
            x_vg = mdl.get_var_by_name('x_{}'.format(vg_idx))
            mdl.add_constraint(x_vs <= x_vg)

        # temporary cast due to expression relations from the dependency graphs
        # these relations state that the precision of a temporary variable 
        # introduced to handle two (or more) operands with different precision
        # must be equal to the minimum precision of the two operands 
        # e.g. V1 = V2+V3 ~= V1 = T4(V2) + T4(V3) --> T4 = min(V2, V3)
        for vt, vv in var_rels['cast_expr']:
            vt_type = vt[0]
            vt_idx = int(vt[1:])
            x_vt = mdl.get_var_by_name('x_{}'.format(vt_idx))
            x_vv = []
            vv_type = []
            vv_idx = []
            vtv_str = 'v{}_'.format(vt_idx)
            for v in vv:
                vv_type.append(v[0])
                vv_idx.append(int(v[1:]))
                x_vv.append(mdl.get_var_by_name('x_{}'.format(int(v[1:]))))
...
'''
def cut_solution(mdl, cut_sol, n_iter, n_wc):
    bin_vars_cut_vals = []
    for i in range(len(cut_sol)):
        x_var = mdl.get_var_by_name('x_{}'.format(i))
        bin_vars_cut_vals.append(mdl.binary_var(name='bvcv_{}_{}_{}'.format(
            n_iter, n_wc, i)))
...
'''
def infer_new_examples(benchmark, sol_stats, err_target):
    new_examples = {}
    conf_dict = {}
    for i in range(len(sol_stats['config'])):
        conf_dict['var_{}'.format(i)] = [sol_stats['config'][i]]
    dif_target = 1 / (sol_stats['error_log'] - err_target)
    if dif_target <= 0:
        new_ex_weight_due_2_dist_from_trgt = 0.01
    elif dif_target > 2:
        new_ex_weight_due_2_dist_from_trgt = 1
    else:
        new_ex_weight_due_2_dist_from_trgt = dif_target
    ...
    new_examples['weights_regr'] = [new_ex_weight_due_2_dist_from_trgt]*(nn+1)
    return new_examples
...
- we do _not_ scrap the old model and we do _not_ start from the beginning
'''
def refine_ML(benchmark, regr, classr, initial_df, new_examples,
    ...
    new_weights_regr = [x * new_config_weight 
            for x in new_examples['weights_regr']]
    new_weights_regr_nparray = np.asarray(new_weights_regr)

    new_train_data_weights_regr = np.hstack(
            [initial_df['weights_regr'],new_weights_regr_nparray])
...
def opt_model_AL(benchmark, initial_df, trgt_error_ratio,
        ...
        debug=True, retrain_ML_fromScratch=False, initial_train_time=0,
        var_rels={}):
    solve_times = []
    ...
    opt_config, mdl, bit_sum = solve_opt_model(benchmark, None,
            ...
            acc, 0, False, train_set_size, large_err_thresh, 0, wrong_configs,
            debug, var_rels)
    if mdl == 1:
        return opt_config

    after_firstSol_solve_time = time.time()
    solve_times.append(after_firstSol_solve_time - before_firstSol_solve_time)

    if opt_config == None:
        print(" ***** MP found no solution *****")
        #return opt_config
        # if the MP model did not find a solution, we provide a simple (probably
        # wrong one) anyway -- it'll be fixed by refine loop
        opt_config = [min_nbit + int((max_nbit - min_nbit)/2)
                ] * benchmark_nVar[benchmark]
    ...
    sol_stats = compute_sol_stats(opt_config, opt_config, prev_sol_stats, error,
            errPred, error_class, acc, mae, r2, ev, after_firstSol_time -
            before_firstSol_time + initial_train_time, debug, True,
            large_err_thresh)
    ...
    while not is_error_se_trgt and n_iter < max_refine_iterations:
        ...
        (regr_ref, mae, rmse, r2, ev, underest_ratio, classr_ref, acc,
                max_pred_err, new_df) = refine_ML(benchmark, regr,
                        classr, initial_df, new_examples, classr_needed, debug)
        if debug:
        ...
        wrong_configs.append(opt_config)
        if debug:
            print("\t----- Search solution with opt model -----")
        opt_config, mdl_ref, bit_sum = solve_opt_model(benchmark, None, 
                trgt_error_ratio_log_exp, regr, mae, rmse, underest_ratio, 
                classr, acc, prev_bit_sum, increase_tot_nbits, 
                train_set_size, large_err_thresh, n_iter, wrong_configs, 
                debug, var_rels)
        if opt_config == None:
            print("[optAL] ***** MP found no solution *****")
            # if the MP model did not find a solution, we provide a simple 
            # (probably wrong one) anyway -- it'll be fixed by refine loop
            opt_config = [v + 1 for v in prev_config]
            bit_sum = sum(opt_config)
        ...
        if not is_error_se_trgt:
            if error_class == 0:
                ...
                n_large_error += 1
        if debug:
            print("[optAL]\tRefined solution at iter {0}: {1}; error {2} < "
                    "error_target {3}? {4}".format(n_iter, opt_config, 
                        error, trgt_error_ratio, is_error_se_trgt))
            print("[optAL]\tActual err (log) {0:.3f}, pred err (log) {1:.3f},"
                    " desired err (log) {2:.3f}; actual class {3}, pred "
                    "class {4}".format(-np.log(error), errPred, 
                        -np.log(trgt_error_ratio), error_class, classPred))
            print("[optAL]\tTime to find refined sol {0:.3f}".format(
                    after_refineSol_solve_time - before_refineSol_solve_time))
            print("[optAL]\tTime to check sol {0:.3f}".format(
    ...
    return opt_config, n_iter, solve_times
...
def main(argv):
    ...
    largeET_classr = trgt_error_ratio
    ############################################################################

    ######################### get dependency graph ############################
    var_rels = {}
    if use_var_relations:
        if benchmark == 'correlation':
            var_rels['bin'], var_rels['cast_expr'] = dep_graphs.get_relations(
                    benchmark, False)
        else:
            var_rels['bin'], var_rels['cast_expr'] = dep_graphs.get_relations(
                    benchmark, True)
    ...
    opt_config, n_iter, solve_times = opt_model_AL(benchmark, initial_df,
            trgt_error_ratio, trgt_error_ratio_log_exp, regr, mae, rmse, r2, ev,
            underest_ratio, classr, acc, initial_train_set_size, largeET,
            input_set_idx, True, retrain_ML_fromScratch, after_initial_time -
            before_initial_time, var_rels)
